...
2. 재생바·미세조정 버튼·오프셋 직접 입력으로 싱크를 맞춤
...
| `hooks/useSyncedPlayers.ts` | 오프셋 기반 동시 재생 + 드리프트 보정 + nudge |
| `hooks/useKeyboardShortcuts.ts` | 편집 모드 키보드 단축키 |
| `components/SyncApp.tsx` | 전체 화면 오케스트레이션 |
| `components/PlayerFrame.tsx` | 단일 YouTube 플레이어 (선택 가능) |
| `components/NudgeControls.tsx` | 영상별 ±초 미세조정 버튼 |
| `components/OffsetControl.tsx` | 오프셋 표시 + 직접 입력 |
